/** Copied from lucid-cardano@0.10.7/src/plutus/data.ts. The compiler complains with an error like:
 * error TS2742: The inferred type of 'MyLocalSchema' cannot be named without a reference to '.../node_modules/lucid-cardano/typebox'
 *
    ...
    try {
      if (typeof data === 'bigint') {
        ...
        return C.PlutusData.new_constr_plutus_data(
          C.ConstrPlutusData.new(C.BigNum.from_str(index.toString()), plutusList)
        );
...
function fromJson(json: Json): Data {
  function toData(json: Json): Data {
    if (typeof json === 'string') {
      return json.startsWith('0x') ? toHex(fromHex(json.slice(2))) : fromText(json);
    }
      ...
      switch (typeof struct) {
        case 'string': {
          ...
          const enumIndex = (shape as TEnum).anyOf.findIndex(
            (s: TLiteral) => s.dataType === 'constructor' && s.fields.length === 0 && s.title === struct
          );
